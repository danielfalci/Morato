...
<?php
	...
	if ( !isset( $product_data['meta']['_wpsc_stock'] ) )
		$product_data['meta']['_wpsc_stock'] = ''; ?>
			...
			<label for='add_form_quantity_limited' class='small'><?php _e( 'Este produto tem estoque limitado', 'wpsc' ); ?></label>
			<?php
	if ( $post->ID > 0 ) {
		if ( is_numeric( $product_data['meta']['_wpsc_stock'] ) ) {?>
					...
					<?php if ( wpsc_product_has_children( $post->ID ) ) : ?>
						...
						<p><?php _e( 'Este produto possui varia&ccedil;&otilde;es, para editar as quantidades use o controle de varia&ccedil;&otilde;es ao lado.' , 'wpsc' ); ?></p>
						<p><?php printf( _n( "%s variant item in stock.", "%s &iacute;tens de varia&ccedil;&atilde;o em estoque.", $stock, 'wpsc' ), $stock ); ?></p>
					<?php else: ?>
					...
					<?php endif; ?>
						<div class='unpublish_when_none_left'>
							...
							<label for="inform_when_oos"><?php _e( 'Notifique o administrador caso o produto fique fora de estoque', 'wpsc' ); ?></label>
						</div>
						<p><em><?php _e( '', 'wpsc' ); ?></em></p>
				</div> <?php
	} else { ?>
