...
if(wpsc_cart_item_count() < 1) :
   _e('<a class="carrinhoVazio" href="http://www.moratoracing.com.br"></a>', 'wpsc') . "<a href=".get_option("product_list_url").">" . __('Please visit our shop', 'wpsc') . "</a>";
   return;
endif;
?>
...
<table class="checkout_cart">
   <tr class="header">
      ...
      <th><?php _e('Pre&ccedil;o', 'wpsc'); ?></th>
      <th>Remover</th>
   </tr>
   <?php while (wpsc_have_cart_items()) : wpsc_the_cart_item(); ?>
      ...
      <tr class="product_row product_row_<?php echo wpsc_the_cart_item_key(); ?> <?php echo $alt_class;?>">
         ...
         <td class="wpsc_product_price wpsc_product_price_<?php echo wpsc_the_cart_item_key(); ?>"><?php echo wpsc_cart_item_price(); ?></td>
         ...
         <td class="wpsc_product_remove wpsc_product_remove_<?php echo wpsc_the_cart_item_key(); ?>">
            <form action="<?php echo get_option('shopping_cart_url'); ?>" method="post" class="adjustform remove">
               ...
               <input type="hidden" name="key" value="<?php echo wpsc_the_cart_item_key(); ?>" />
               <input type="hidden" name="wpsc_update_quantity" value="true" />
               <input type="submit" class="removeBotao" value="<?php _e('', 'wpsc'); ?>" name="submit" />
            </form>
         </td>
      </tr>
   <?php endwhile; ?>
  ...
  <?php if(wpsc_uses_shipping()): ?>
   ...
   <?php if(wpsc_uses_shipping()) : ?>
		...
		<thead>
		     <tr class="wpsc_shipping_info header">
		     	<th><?php _e('Calculate Shipping Price', 'wpsc'); ?></th>
		     	<th>Postagem</th>
		     	<th></th>
		     </tr>
		</thead>
         ...
         <?php if (!wpsc_have_shipping_quote()) : // No valid shipping quotes ?>
            <?php if (wpsc_have_valid_shipping_zipcode()) : ?>
                  <tr class='wpsc_update_location'>
                     <td colspan='3' class='shipping_error' >
         ...
         <tr class='wpsc_change_country'>
            <td>
               <form name='change_country' id='change_country' action='' method='post'>
                  ...
                  <input type='submit' name='wpsc_submit_zipcode' value='Calcular' />
               </form>
            </td>
        
		<?php if($_SESSION['wpsc_zipcode'] != ""){ ?>
...
?>
   ...
   <table  class='wpsc_checkout_table table-4 checkout_cart'>
      <?php if(wpsc_uses_shipping()) : ?>
	      <tr class="header">
      ...
      <div class='wpsc_make_purchase'>
         <span>
            ...
            <?php endif; ?>
               ...
               <button type='submit' name='submit' class='make_purchase wpsc_buy_button botaopagar'>
               		<img src="<?php bloginfo('template_directory'); ?>/images/icnBtnSeguir.png" alt="icnBtnSeguir" width="16" height="16" /> Pagar com               				</button>
         </span>
      </div>
